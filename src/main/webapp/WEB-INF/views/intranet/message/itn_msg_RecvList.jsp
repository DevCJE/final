...
<br><br><br><br><br>
	<div class="content">
		<ul class="nav nav-tabs">
			...
			<li><a href="${itnMsgUrl}">받은쪽지함</a></li>
			<c:url var="itnMsgRecvUrl" value="itn_msg_RecvList.do">
				<c:param name="recv_name">${sessionScope.name}</c:param>
			</c:url>
			<li class="active"><a href="${itnMsgRecvUrl}">보낸쪽지함</a></li>
		</ul>
		<form name="recvForm" action="itn_msg_RecvDel.do" method="post">
			<table border="0" cellspacing="0" class="table table-hover">
				<thead>
					<tr align="center">
						...
						<td width="50px">날짜</td>
						<td width="100px">수신여부</td>
						<td width="100px">발송취소</td>
					</tr>
				</thead>
				<tbody>
					<c:if test="${empty list}">
						<tr align="center">
							<td colspan="7" align="center">보낸 쪽지가 없습니다.</td>
						</tr>
					</c:if>
					<c:forEach var="dto" items="${list}">
						<c:if test="${dto.recv_name == sessionScope.name}">
							<c:if test="${dto.recv_del != 'Y'}">
								<tr align="center">
									<td><input type="checkbox" name="chk" value="${dto.idx}"></td>
									<td>${dto.idx}</td>
									<td>${dto.send_name}</td>
									<c:url var="msgContentUrl" value="itn_msg_RecvContent.do">
									...
									<c:if test="${dto.message_view==0}">
										<td>미확인</td>
										<td><c:url var="cancelUrl" value="itn_msg_Cancel.do">
												<c:param name="idx">${dto.idx}</c:param>
												<c:param name="recv_name">${sessionScope.name}</c:param>
											</c:url>
											<button type="button" name="cancel" class="btn btn-warning btn-xs"
												onclick="location.href='${cancelUrl}'">발송취소</button></td>
									</c:if>
									<c:if test="${dto.message_view>0}">
										<td>${dto.message_date}</td>
										<td>수신완료</td>
					...
					<tr align="right">
						<td colspan="7"><input type="hidden" name="recv_name" value="${sessionScope.name}"> 
						<input type="submit" class="btn btn-warning" value="삭제하기"></td>
					</tr>
				</tbody>
